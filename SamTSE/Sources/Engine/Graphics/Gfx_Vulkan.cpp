...
BOOL CGfxLibrary::InitDriver_Vulkan()
{
  ...
  CPrintF("Vulkan: new SvkMain...\n");
  result = gl_SvkMain->InitDriver_Vulkan();
  return result;
}
...
BOOL SvkMain::InitDriver_Vulkan()
{
  ...
  else if (r != VK_SUCCESS)
  {
    ...
    CPrintF("Vulkan: Create vkCreateInstance Done.\n");
  }
  ...
  if (pfnCreateDUMsg != nullptr) 
  {
    ...
    CPrintF("Vulkan: vkCreateDebugUtilsMessengerEXT Done.\n");
  }
  else 
  {
...
#else
  if (InitSurface_SDL(instanceInfo, _hwndMain) == SDL_FALSE)
  {
    CPrintF("Vulkan error: Can't create SDL2 surface.\n");
#endif
    return FALSE;
  } else {
    CPrintF("Vulkan: Create SDL2 surface Done.\n");
  }
  ...
  CPrintF("Vulkan: Try PickPhysicalDevice...\n");
  if (!PickPhysicalDevice())
  {
    CPrintF("Vulkan error: Can't find suitable physical device.\n");
    return FALSE;
  } else {
    CPrintF("Vulkan: Suitable physical device is present.\n");
  }
  ...
  if (!CreateDevice())
  {
    ...
    CPrintF("Vulkan: Create VkDevice Done.\n");
  }

  CPrintF("Vulkan: Try InitSamplers...\n");
  InitSamplers();
  CPrintF("Vulkan: Try CreateTexturesDataStructure...\n");
  CreateTexturesDataStructure();
  CPrintF("Vulkan: Try CreateDescriptorPools...\n");
  CreateDescriptorPools();
  CPrintF("Vulkan: Try CreateCmdBuffers...\n");
  CreateCmdBuffers();
  CPrintF("Vulkan: Try CreateSyncPrimitives...\n");
  CreateSyncPrimitives();
  CPrintF("Vulkan: Try CreateVertexLayouts...\n");
  CreateVertexLayouts();
  CPrintF("Vulkan: Try CreatePipelineCache...\n");
  CreatePipelineCache();
  CPrintF("Vulkan: Try CreateRenderPass...\n");
  CreateRenderPass();
  CPrintF("Vulkan: Try CreateDescriptorSetLayouts...\n");
  CreateDescriptorSetLayouts();
  CPrintF("Vulkan: Try CreateShaderModules...\n");
  CreateShaderModules();
  CPrintF("Vulkan: Try InitDynamicBuffers...\n");
  InitDynamicBuffers();
  CPrintF("Vulkan: Try InitOcclusionQuerying...\n");
...
#else
SDL_bool SvkMain::InitSurface_SDL(VkInstanceCreateInfo InstanceIn, SDL_Window * _hwndMain)
{
    SDL_bool r = SDL_Vulkan_CreateSurface(_hwndMain, gl_VkInstance, &gl_VkSurface);
    return r;
}
#endif
...
void SvkMain::AcquireNextImage()
{
  ...
  if (r == VK_ERROR_OUT_OF_DATE_KHR)
  {
    ...
    CPrintF("Vulkan: // TODO: Vulkan: recreate swapchain!\n");
  }
